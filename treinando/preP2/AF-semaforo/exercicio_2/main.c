...
// ====================== ATENCAO -- ESSA BOMBA DE CÓDIGO TA ERRADA <3 ========================================
...
void ponte_inicializar() {
	...
	sem_init(&way_semaphores[CONTINENTE], 0, veiculos_turno);
	sem_init(&way_semaphores[ILHA], 0, 0);
	sairam = 0;

	for (int i = 0; i < veiculos_turno; i++) {
    	sem_post(&way_semaphores[CONTINENTE]);
	}
...
void ponte_entrar(veiculo_t *v) {
	...
	sem_wait(&way_semaphores[v->cabeceira]);
...
void ponte_sair(veiculo_t *v) {
	...
	if (sairam == veiculos_turno) {
		...
		printf("\n[PONTE] *** Novo sentido da travessia: %s -> %s. ***\n\n", cabeceiras[!direction], cabeceiras[direction]);
		fflush(stdout);

		for (int i = 0; i < veiculos_turno; i++) {
			sem_post(&way_semaphores[direction]);
		}
...
void ponte_finalizar() {
	...
	sem_destroy(&way_semaphores[CONTINENTE]);
	sem_destroy(&way_semaphores[ILHA]);
